...
include_once($phpbb_root_path . 'rpg/database/RPGKarmaTopics.class.' . $phpEx);
include_once($phpbb_root_path . 'rpg/database/RPGPVPBattles.class.' . $phpEx);
include_once($phpbb_root_path . 'rpg/database/RPGRPForums.class.' . $phpEx);
...
$template->assign_vars(array(
	...
	//modif ici pour afficher ou non le bouton répondre suivant si le joueur peut rp ou pas sur ce topic
	'S_DISPLAY_REPLY_INFO'	=> (!RPGKarmaTopics::userHasEndRP($forum_id, $topic_id, $user->data['user_id']) && $topic_data['forum_type'] == FORUM_POST && ($auth->acl_get('f_reply', $forum_id) || $user->data['user_id'] == ANONYMOUS)) ? true : false,
	...
	//rpg modif
	'S_DISPLAY_PVP'			=> RPGPVPBattles::forumIsPVP($forum_id) and (RPGKarmaTopics::getNumberOfPostsInTopic($forum_id, $topic_id, $user->data['user_id']) > 0) and ($user->data['user_id'] != ANONYMOUS) and ($topic_data['topic_status'] != ITEM_LOCKED),
	'U_POST_PVP'			=> append_sid("javascript:open_pvp_request_window('" . append_sid("{$phpbb_root_path}startbattle.php", "mode=getopponents&amp;f=$forum_id&amp;t=$topic_id") . "')"),
	'S_DISPLAY_KARMA'		=> RPGRPForums::forumIsRP($forum_id) and (RPGKarmaTopics::getNumberOfPostsInTopic($forum_id, $topic_id, $user->data['user_id']) > 0) and !RPGKarmaTopics::userHasEndRP($forum_id, $topic_id, $user->data['user_id']),
	'U_POST_KARMA'			=> append_sid("javascript:open_karma_window('" . append_sid("{$phpbb_root_path}endrp.php", "f=$forum_id&amp;t=$topic_id") . "')"),
	//rpg end

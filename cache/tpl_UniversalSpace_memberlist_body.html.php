<?php if (!defined('IN_PHPBB')) exit; if ($this->_rootref['S_IN_SEARCH_POPUP']) {  $this->_tpl_include('simple_header.html'); } else { $this->_tpl_include('overall_header.html'); } if ($this->_rootref['S_SEARCH_USER']) {  $this->_tpl_include('memberlist_search.html'); } if ($this->_rootref['S_SHOW_GROUP']) {  $this->_tpl_include('memberlist_group.html'); } if (! $this->_rootref['S_SHOW_GROUP']) {  ?>
	<form method="post" name="charsearch" action="<?php echo (isset($this->_rootref['S_MODE_ACTION'])) ? $this->_rootref['S_MODE_ACTION'] : ''; ?>">
		...
		<tr>
			<td align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>"><span class="genmed"><?php echo ((isset($this->_rootref['L_USERNAME_BEGINS_WITH'])) ? $this->_rootref['L_USERNAME_BEGINS_WITH'] : ((isset($user->lang['USERNAME_BEGINS_WITH'])) ? $user->lang['USERNAME_BEGINS_WITH'] : '{ USERNAME_BEGINS_WITH }')); ?>: </span>
				<select name="first_char" onchange="this.form.submit();">
					<?php $_first_char_count = (isset($this->_tpldata['first_char'])) ? sizeof($this->_tpldata['first_char']) : 0;if ($_first_char_count) {for ($_first_char_i = 0; $_first_char_i < $_first_char_count; ++$_first_char_i){$_first_char_val = &$this->_tpldata['first_char'][$_first_char_i]; ?>
						<option value="<?php echo $_first_char_val['VALUE']; ?>"<?php if ($_first_char_val['S_SELECTED']) {  ?> selected="selected"<?php } ?>><?php echo $_first_char_val['DESC']; ?></option>
					<?php }} ?>
				</select>&nbsp;<input type="submit" name="char" value="<?php echo ((isset($this->_rootref['L_DISPLAY'])) ? $this->_rootref['L_DISPLAY'] : ((isset($user->lang['DISPLAY'])) ? $user->lang['DISPLAY'] : '{ DISPLAY }')); ?>" class="btnlite" /></td>
...
<tr>
	<td class="cats" colspan="<?php if ($this->_rootref['S_IN_SEARCH_POPUP']) {  ?>9<?php } else { ?>8<?php } ?>"></td>
</tr>
<tr>
	<th nowrap="nowrap">#</th>
...
<?php } ?>
<tr>
	<td class="cati" colspan="<?php if ($this->_rootref['S_IN_SEARCH_POPUP']) {  ?>9<?php } else { ?>8<?php } ?>" align="center"></td>
</tr>

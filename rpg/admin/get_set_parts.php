...
if($type == '') { echo 'error'; return; }

switch($type) {
	case 'clothes':
		$p = RPGClothes::getClothes();
		break;
	case 'leggings':
		$p = RPGLeggings::getLeggings();
		break;
	case 'gloves':
		$p = RPGGloves::getGloves();
		break;
	case 'shoes':
		$p = RPGShoes::getShoes();
		break;
...
foreach($p as $part) {
	$html .= "<option value=\"{$part->getId()}\">{$part->getName()}</option>" . PHP_EOL;
}

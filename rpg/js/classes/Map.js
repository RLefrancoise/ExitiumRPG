function Map(nom) {
	...
	var req = $.getJSON("rpg/maps/" + nom + ".json", function(data){
...
Map.prototype.characterIsOnMap = function(id) {
	for(var i = 0 ; i < this.characters.length ; i++) {
		if(this.characters[i].accountData.id == id) return true;
	}
...
Map.prototype.getCharacter = function(id) {
	for(var i = 0 ; i < this.characters.length ; i++) {
		if(this.characters[i].accountData.id == id) return this.characters[i];
	}

function open_centered_window(page, name, largeur,hauteur,options) {
  var top=(screen.height-hauteur)/2;
  var left=(screen.width-largeur)/2;
  var w = window;
  
  return w.open(page,name,"top="+top+",left="+left+",width="+largeur+",height="+hauteur+","+options);
}

function open_pve_window(sid, area, area_part) {
	return open_centered_window('startbattle.php?sid=' + sid + '&mode=pve&a=' + area + '&p=' + area_part, 'Combat', 800, 600, 'menubar=no,statusbar=no,location=no');
}

function open_battle_window(link) {
	open_centered_window(link, 'Combat', 800, 600, 'menubar=no,statusbar=no,location=no');
}

function open_pvp_request_window(link) {
	open_centered_window(link, 'PVP', 400, 300, 'menubar=no,statusbar=no,location=no');
}

function open_karma_window(link) {
	var c = confirm('Terminer le RP ?');
	if(c == true) {
		open_centered_window(link, 'Mettre fin au RP', 400, 300, 'menubar=no,statusbar=no,location=no');
	}
}

function open_event_register_window(link) {
	open_centered_window(link, 'Inscription à un event', 400, 300, 'menubar=no,statusbar=no,location=no');
}

function open_quest_window(quest_id) {
	var c = confirm('Démarrer cette quête ?');
	if(c == true) {
		open_centered_window('./startquest.php?q=' + quest_id, 'Démarrer une quête', 400, 300, 'menubar=no,statusbar=no,location=no');
	}
}
...
function put_menu_and_chatbox(sid) {
	var html = 	'<div id="rpgmenu_div" style="position:fixed;top:200px;left:-30px;z-index:5">\
					<iframe id="rpgmenu" src="./viewrpgmenu.php' + ((sid != undefined) ? ('?sid=' + sid) : '') + '" width="262" height="604" style="display:none;float:left;border:none"></iframe>\
